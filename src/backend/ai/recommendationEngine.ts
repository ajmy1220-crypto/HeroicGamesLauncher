 ...
 */
export function recommend(analysis: LogAnalysis, context: GameContext): Recommendation {
    ...
    result.actions = [
      {
        ...
        params: {},
        reason:
          summary.topSignal?.message ?? '偵測到硬不相容（如反作弊），同組設定不必重試',
        autoApplyable: false,
        confidence: summary.topSignal?.confidence ?? BLOCKING_FALLBACK_CONFIDENCE,
      },
    ];
    return result;
  }
  ...
  if (!isCurrentAdequate(context, dx)) {
    if (summary.suggestedBackend) {
      ...
      targetConfidence = summary.topSignal?.confidence ?? CAPABILITY_ONLY_CONFIDENCE;
  ...
  if (target && dx !== undefined && !supports(target, dx, context.arch)) {
    ...
    target = fallback;
    if (fallback) targetConfidence = activeSwitchConfidence(summary.topSignal?.confidence);
  }
  ...
  if (osSig && context.currentBackend === 'dxmt' && isSonomaOrOlder(context.osVersion)) {
    result.actions.push({
      kind: 'none',
      params: {},
      reason:
 ...
 *     避免對能跑的設定無謂折騰。
 */
function isCurrentAdequate(context: GameContext, dx: DirectXVersion | undefined): boolean {
...
function isArchMismatch(backend: GameContext['currentBackend'], context: GameContext): boolean {
  if (context.arch === 'x86_64' && (backend === 'dxmt' || backend === 'gptk')) return true;
  return false;
}
...
function dedupeKey(act: RecommendedAction): string {
  if (act.kind === 'switch_backend') return `switch_backend:${String(act.params.backend)}`;
  if (act.kind === 'install_winetricks') return `install_winetricks:${String(act.params.verb)}`;
  // none 的提示型：以 suggestedBackend（若有）區隔 Steam/crossover 提示 vs 占位。
  if (act.kind === 'none') return `none:${String(act.params.suggestedBackend ?? '')}:${act.reason}`;
