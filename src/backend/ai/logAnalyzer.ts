...
const RULES: SignalRule[] = [
  // ── 反作弊：最該先攔，攔到就別浪費 retry ──────────────────────
  {
    ...
    patterns: [/BattlEye/i, /EasyAntiCheat|\bEAC\b/i, /Denuvo/i, /nProtect|GameGuard/i],
    confidence: 0.9,
    hint: { verdict: 'incompatible', blocking: true },
    describe: () => '偵測到反作弊元件（BattlEye / EAC / Denuvo 等），Wine 環境下通常無法執行',
  },
  ...
  // ── 缺 DLL / 執行庫：Wine debug channel ──────────────────────
  {
    ...
    describe: (d) => `缺少程式庫：${d?.dll ?? '(未知)'}，多半可用 winetricks 補上`,
  },
  ...
  // ── .NET / VCRedist 這類最常見的執行庫缺漏 ──────────────────
  {
    ...
    confidence: 0.7,
    hint: { verdict: 'recoverable' },
    describe: () => '缺少常見執行庫（.NET / VCRedist / d3dcompiler），通常可一鍵補裝',
  },
  ...
  // ── DXVK：建立 D3D 裝置失敗（最常見的「跑不起來」）─────────
  {
    ...
    describe: () => 'D3D 裝置建立失敗（多半是後端或 feature level 不符），建議改用其他圖形後端',
  },
  ...
  // ── 把 DX12 跑在只支援 DX10/11 的後端上（DXMT/DXVK-macOS）──
  {
    ...
    describe: () => '偵測到 DirectX 12 需求；DXMT / DXVK-macOS 只支援到 DX11，需改用 GPTK',
  },
  ...
  // ── 把 D3D9 跑在 DXMT 上（DXMT 只做 D3D11/D3D10）─────────────
  {
    ...
    patterns: [/d3d9(?:\.dll)?|Direct3DCreate9(?:Ex)?|IDirect3D(?:Device)?9|DirectX 9/i],
    confidence: 0.6,
    ...
    describe: () => '偵測到 DirectX 9 需求；DXMT 不支援 D3D9，建議改用 DXVK-macOS（支援到 DX9）',
  },
  ...
  // ── DXMT：不支援的功能（Stream-Output 等）────────────────────
  {
    ...
    confidence: 0.5,
    hint: { verdict: 'needs_backend_change' },
    describe: () => 'DXMT 尚未支援此功能（如 Stream-Output / 部分 tessellation），可改試其他後端',
  },
  ...
  // ── DXMT / 通用：shader 不支援或 airconv 失敗 ────────────────
  {
    ...
    confidence: 0.5,
    hint: { verdict: 'needs_backend_change' },
    describe: () => 'Shader 編譯失敗或不支援（DXMT airconv 轉譯問題），可能造成崩潰或算繪錯誤',
  },
  ...
  // ── DXMT：macOS 版本不符（Sonoma 的 Metal 3.2 intrinsic）────
  {
    ...
    confidence: 0.4,
    hint: { verdict: 'recoverable' },
    describe: () => 'DXMT 在 macOS Sonoma 上有 Metal 3.2 shader intrinsic 的相容問題，建議升級到 Sequoia 以上',
  },
  ...
  // ── 啟動器崩潰：Epic Online Services ─────────────────────────
  {
    ...
    confidence: 0.5,
    hint: { verdict: 'recoverable' },
    describe: () => '偵測到 Epic Online Services；舊版 DXMT 曾導致從 Epic 啟動的遊戲崩潰，請確認後端為新版',
  },
  ...
  // ── 致命崩潰：page fault / unhandled exception ───────────────
  {
    ...
    describe: () => '行程因未處理的例外或 page fault 崩潰，需結合上文 evidence 進一步判斷',
  },
  ...
  // ── 效能訊號（非失敗，可調）：PSO / shader 快取卡頓 ─────────
  {
    ...
    describe: () => '偵測到 PSO / shader 快取相關訊息：首次執行的卡頓多半會在後續啟動改善',
  },
];
 ...
 */
export function analyze(raw: string, context: AnalyzerContext = {}): LogAnalysis {
  ...
  for (const line of lines) {
    ...
    for (const rule of RULES) {
      ...
      if (existing) {
        if (existing.evidence.length < maxEvidence) existing.evidence.push(line.trim());
        continue;
      }
...
function rollup(signals: DetectedSignal[], context: AnalyzerContext): AnalysisSummary {
  ...
  for (const sig of signals) {
    ...
    if (hint.verdict) {
      merged = merged === undefined ? hint.verdict : mergeVerdict(merged, hint.verdict);
    }
  }
...
const SEVERITY_ORDER: Record<Severity, number> = { fatal: 0, error: 1, warning: 2, info: 3 };

function bySeverityThenConfidence(a: DetectedSignal, b: DetectedSignal): number {
